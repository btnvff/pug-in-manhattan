...
const WorldRatio = (() => {
  ...
  function project(x,y,d){ if(!(d>0)) throw new RangeError('Positive camera depth required'); return {u:C.vanishing_u+C.pug_height_ratio/A*x/d,v:C.horizon_v+(C.pug_ground_v-C.horizon_v-C.pug_height_ratio*y)/d}; }
  function depthAtGround(v){ if(!(v>C.horizon_v)) throw new RangeError('Ground inverse requires v below horizon'); return (C.pug_ground_v-C.horizon_v)/(v-C.horizon_v); }
  ...
  function bounds(mesh,T){mesh.updateMatrixWorld(true);const b=new T.Box3(),v=new T.Vector3();mesh.traverseVisible(n=>{if(n.isMesh&&!/shadow|contact|breath/.test(n.name)){const p=n.geometry.attributes.position;for(let i=0;i<p.count;i++)b.expandByPoint(v.fromBufferAttribute(p,i).applyMatrix4(n.matrixWorld));}});return b;}
...
const RatioPresentation = (() => {
  ...
  // Existing 390×844 simulation remains untouched. A uniform vertical adapter
  // aligns the old 85-unit mouth/catch region with the normalized character.
  ...
  return Object.freeze({px,py,logicalY,unit,heroScale:1/h,foodScale:C.object_dimension_registry.sausage.length/C.assets.sausage.source_long_axis});
})();
