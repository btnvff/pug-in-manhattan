// Procedural assets use authored local units; the ratio adapter normalizes their placement.
...
function createModelFactory() {
  ...
  const geometries = {};
  try {
    geometries.ball = new T.SphereGeometry(1, 16, 12);
    geometries.box = new T.BoxGeometry(1, 1, 1);
    geometries.tube = new T.CylinderGeometry(1, 1, 1, 12);
    geometries.cone = new T.ConeGeometry(1, 1, 12);
    geometries.ring = new T.TorusGeometry(1, 0.25, 8, 24);
    geometries.capsule = new T.CapsuleGeometry(1, 2, 6, 12);
    ...
    for (let row = 0; row <= 32; row++) {
      ...
      for (let col = 0; col <= 16; col++) {
        ...
        casing.setXYZ(index, center.x + (casing.getX(index) - center.x) * radius, center.y + (casing.getY(index) - center.y) * radius, casing.getZ(index) * radius);
      }
    }
    ...
    function foodModel(type, variant = 0) {
      ...
      if (type === 0) {
        const casingRoot = group(root);
        casingRoot.rotation.z = .55;
        ...
        for (const side of [-1, 1])
          part(root, "ball", "#fff1d6", side * 4, -24, 0, 7, 6, 6);
        ...
        for (let i = 0; i < 3; i++)
          rod(root, [-4 + i, 12 - i * 10, 8 - i * 2.2], [4 - i, 11 - i * 10, 8 - i * 2.2], .55, "#ba652e");
      }
      else if (type === 7) {
    ...
    function catModel(coat = 0) {
      ...
      for (const side of [-1, 1]) {
        ...
        for (let n = 0; n < 2; n++)
          rod(head, [side * 6, -4 - n * 2, 10], [side * 14, -3 - n * 4, 8], .23, "#ddd3b6");
      }
      part(head, "ball", "#875a56", 0, -3.6, 12, 1.9, 1.3, .8);
      for (const side of [-1, 1]) {
        const mark = part(head, "ball", stripe, side * 3.3, 7, 8, .9, 2.4, .4);
        mark.rotation.z = side * .3;
      }
      for (let i = 0; i < 3; i++) {
        const mark = part(spine, "ball", stripe, -12 + i * 8, 5, 9.2, 1.5, 4.8, .8);
        mark.rotation.z = -.18;
      }
    ...
    function dispose() {
      if (disposed)
        return;
      disposed = true;
      disposeThreeResources([...Object.values(geometries), ...materials.values()]);
      materials.clear();
    }
    return { part, rod, geometries, foodModel, catModel, birdModel, dispose };
  }
  catch (error) {
    disposeThreeResources([...Object.values(geometries), ...materials.values()]);
    throw error;
...
// Continue releasing independent owners even when one resource fails to dispose.
function disposeThreeResources(resources) {
  for (const resource of resources) {
    try {
      ...
      console.warn("Could not dispose a 3D resource.", error);
    }
  }
}
