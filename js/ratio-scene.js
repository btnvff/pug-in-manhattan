...
function createRatioLayout() {
  ...
  function shape(kind,color,x,y,d,width,height,length,extra={}) {
    ...
    if(![x,y,d,width,height,length].every(Number.isFinite)||d<=0)throw Error('Invalid world descriptor');objects.push(o);return o;
  }
  ...
  for(const side of [-1,1]) {
    ...
    for(let z=transition+1;z<70;z+=1.35)box('#a7a797',side*3.1,.077,z,1.13,.009,.016);
  }
  for(let z=transition+.8;z<100;z+=3.6)box('#e2c875',0,.009,z,.052,.018,1.8);
  // Real ground-plane zebra stripes across the avenue at the intersection.
  for(let x=-2.3;x<2.5;x+=.48)box('#e6dfc8',x,.015,transition-.62,.27,.025,1.0);
  for(let i=0;i<6;i++)box('#ddd6c0',-4.1-i*.5,.012,crossing+1.8,.28,.024,1.7);
  // Deterministic asphalt grain and seams: the art does not consume gameplay RNG.
  for(let i=0;i<200;i++) {const z=.4+(i*1.713%30),x=(Math.sin(i*93.1)*.5+.5)*12-6;box(i%2?'#92978f':'#737c7a',x,.002,z,.016+(i%3)*.012,.006,.027);}
  ...
  for(const b of buildings){
    ...
    for(let q=z+.8;q<z+b.l-.8;q+=1.6){box('#294b50',inner-side*.06,1.25,q,.18,1.85,1.25);box('#d7b98c',inner-side*.19,.24,q,.3,.28,1.4);}
    for(let x=b.x-b.w/2+.65;x<b.x+b.w/2-.3;x+=1.35)box('#294c51',x,1.2,z-.04,1.06,1.9,.16);
    box(b.id.includes('left')?'#426d60':'#963f38',b.x,2.12,z-.44,b.w-.35,.13,.86);
    for(let x=b.x-b.w/2+.1;x<b.x+b.w/2;x+=.48)box('#e5d4ac',x,2.14,z-.47,.19,.04,.86);
    if(b.sign)shape('sign','#e8dab7',b.x,2.44,(z-.17)/Z,b.w-.45,.36,.04,{text:b.sign});
    // Fire escapes attached to street-facing facade coordinates.
    if(b.id==='near-left')for(let y=4;y<12;y+=2.8){
      box('#3e4947',inner+.32,y,z+3.5,.65,.055,1.9);
      for(const q of [z+2.6,z+4.4])rod('#394745',[inner+.65,y,q],[inner+.65,y+.55,q],.025);
      rod('#394745',[inner+.65,y+.55,z+2.6],[inner+.65,y+.55,z+4.4],.025);
      rod('#394745',[inner+.48,y,z+2.7],[inner+.48,y+2.8,z+4.3],.028);
      for(let i=0;i<8;i++)rod('#44504d',[inner+.1,y+i*.35,z+2.7+i*.2],[inner+.5,y+i*.35,z+2.7+i*.2],.022);
    }
    if(b.id==='near-right'){box('#77584b',b.x+.3,b.h+.8,z+2.4,1.15,1.5,1.15);shape('cone','#635347',b.x+.3,b.h+1.85,(z+2.4)/Z,1.5,.65,1.5);}
  }
  ...
  for(let i=0;i<17;i++){
    ...
    for(let y=5;y<h-1;y+=2.2)box('#b7cbd0',x,y,z-1.94,w*.85,.12,.03);
    if(i===8){box(tint,x,52,z,1.3,3,1.6);rod('#bed0d2',[x,53,z],[x,60,z],.11);}
  }
  // Elevated transverse bridge, deck at Y=8.25; piers never form a street gate.
  const bridgeD=12,bridgeZ=bridgeD*Z,deck=C.ground_regions.bridge_deck_y;
  for(const d of [12,14.2]){
    ...
    for(let x=-17;x<=17;x+=.6){const y=deck+1.3+15*Math.pow((Math.abs(x)-.2)/17,2);rod('#73939e',[x,deck+.3,z-1.1],[x,y,z-1.1],.028);if(x<17){const nx=x+.6,ny=deck+1.3+15*Math.pow((Math.abs(nx)-.2)/17,2);rod('#537e91',[x,y,z-1.1],[nx,ny,z-1.1],.07);}}
  }
  ...
  for(const side of [-1,1])for(let i=0;i<4;i++){
    ...
    rod('#455854',[xx,.07,z],[xx,3.3,z],.055);rod('#455854',[xx,3.3,z],[xx-side*.5,3.44,z],.045);
    shape('ball','#f3dca0',xx-side*.5,3.36,d,.26,.15,.26);
    if(i%2===0){box('#637b60',side*3.43,.36,z+1.5,.40,.58,.45);rod('#77654d',[side*3.36,.07,z+3],[side*3.36,2.7,z+3],.065);ball('#6f9472',side*3.36,3,z+3,1.25,1.65,1.2);}
  }
  for(const side of [-1,1]){
...
function createRatioWorld(model){
  const T=window.THREE,root=new T.Group(),owned=[],actors=[];
  root.name='ratio-world';root.userData.space='WORLD';
  const geometries={box:new T.BoxGeometry(1,1,1),ball:new T.SphereGeometry(.5,12,8),cylinder:new T.CylinderGeometry(.5,.5,1,12),cone:new T.ConeGeometry(.5,1,12)};owned.push(...Object.values(geometries));
  const materials=new Map(),buckets=new Map(),matrix=new T.Matrix4(),quat=new T.Quaternion(),pos=new T.Vector3(),scl=new T.Vector3(),up=new T.Vector3(0,1,0);
  function material(color){if(!materials.has(color))materials.set(color,new T.MeshStandardMaterial({color,roughness:.9}));return materials.get(color);}
  for(const o of RATIO_LAYOUT.objects){
    if(o.kind==='sign'){
      if(ratioMode==='blockout')continue;
      const canvas=document.createElement('canvas');canvas.width=512;canvas.height=64;const pen=canvas.getContext('2d');pen.fillStyle='#284f4b';pen.fillRect(0,0,512,64);pen.fillStyle=o.color;pen.font='bold 40px Georgia';pen.textAlign='center';pen.textBaseline='middle';pen.fillText(o.text,256,34,490);
      const texture=new T.CanvasTexture(canvas);texture.colorSpace=T.SRGBColorSpace;const mat=new T.MeshBasicMaterial({map:texture});const geo=new T.PlaneGeometry(o.width,o.height);owned.push(texture,mat,geo);const mesh=new T.Mesh(geo,mat);mesh.position.set(o.x,o.y,-o.d*5.625);root.add(mesh);continue;
    }
    if(ratioMode==='blockout'&&!o.role&&!o.surface)continue;
    const kind=o.kind==='rod'?'cylinder':o.kind,key=kind+':'+o.color;
    if(!buckets.has(key))buckets.set(key,{kind,color:o.color,objects:[]});buckets.get(key).objects.push(o);
  }
  for(const batch of buckets.values()){
    const mesh=new T.InstancedMesh(geometries[batch.kind],material(batch.color),batch.objects.length);
    batch.objects.forEach((o,i)=>{pos.set(o.x,o.y,-o.d*5.625);quat.identity();if(o.kind==='rod'){const v=new T.Vector3(o.b[0]-o.a[0],o.b[1]-o.a[1],o.a[2]-o.b[2]);quat.setFromUnitVectors(up,v.normalize());}scl.set(o.width,o.height,o.length);matrix.compose(pos,quat,scl);mesh.setMatrixAt(i,matrix);});root.add(mesh);
  }
  // Original sedan silhouette, authored once in canonical dimensions; camera scales it.
  function car(color,d,lane){
    const g=new T.Group();g.userData={space:'WORLD',dimensions:COPY_TAXI(),d,lane};
    ...
    for(const side of [-1,1]){
      for(const z of [-1.25,1.24]){part('ball','#283333',side*.82,.25,z,.18,.25,.25);part('ball','#abb2a7',side*.97,.25,z,.025,.12,.12);}
    ...
    root.add(g);actors.push({kind:'car',mesh:g,d,lane});return g;
  }
  function COPY_TAXI(){return {...PUG_WORLD_RATIO.object_dimension_registry.taxi};}
  if(ratioMode!=='blockout'){
    car('#eab73c',4.8,-1.25);car('#698395',7.5,1.25);
    for(let i=0;i<5;i++){
      ...
      const legs=[];for(const s of [-1,1])legs.push(model.rod(g,[s*.1,1,0],[s*.1,.07,0],.063,'#344a53'));
      for(const s of [-1,1])model.rod(g,[s*.19,1.55,0],[s*.23,.99,0],.057,'#a77f62');
      root.add(g);actors.push({kind:'person',mesh:g,d,lane:side*3.16,legs});
    }
  }
  function animate(time){for(const a of actors){const z=(a.d+(a.kind==='car'?(Math.sin(time*.055+a.d)-Math.sin(a.d))*.55:(Math.sin(time*.07+a.d)-Math.sin(a.d))*.22))*5.625;a.mesh.position.set(a.lane,0,-z);if(a.kind==='person')a.legs.forEach((leg,i)=>{leg.rotation.x=Math.sin(time*3+i*Math.PI)*.22;});}}
  animate(0);
  return {root,animate,actors,dispose(){root.traverse(o=>{if(o.isInstancedMesh)o.dispose();});owned.forEach(x=>x.dispose());materials.forEach(x=>x.dispose());root.clear();}};
}
